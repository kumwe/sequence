...
namespace Kumwe\Sequence\Tests;
...
abstract class TestCase
{
     ...
     * @param   string  $subject  Text under test.
     * @param   string  $message  Why the shape matters, reported on failure.
     *
     * @return  void
     *
     * @throws  \RuntimeException  When the subject does not match.
     *
     * @since   0.1.0
     */
    final protected function assertMatches(string $pattern, string $subject, string $message): void
    {
        $this->assertions++;
        if (preg_match($pattern, $subject) !== 1) {
            throw new \RuntimeException($message . ' ' . var_export($subject, true) . ' does not match ' . $pattern);
        }
    }
