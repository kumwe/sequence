 ...
 *
 * Assertions come from Kumwe\Sequence\Tests\TestCase. No framework, so the suite runs on any supported PHP
 * with no composer install. Discovery fails closed: deleting the suite, or leaving a discovered case with no
 * test methods, is a build failure rather than an empty success.
 *
 * @since 0.1.0
 */
...
spl_autoload_register(static function (string $class): void {
    $prefixes = [
        'Kumwe\\Sequence\\Tests\\' => __DIR__ . '/',
        'Kumwe\\Sequence\\' => dirname(__DIR__) . '/src/',
    ];
...
if ($files === []) {
    fwrite(STDERR, "Sequence suite failed: no test case files were discovered.\n");
    exit(1);
}
...
foreach ($files as $file) {
    $class = 'Kumwe\\Sequence\\Tests\\Case\\' . basename($file, '.php');
...
echo "\nSequence suite passed: {$totalTests} tests, {$totalAssertions} assertions.\n";
