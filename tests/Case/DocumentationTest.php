...
final class DocumentationTest extends TestCase
{
    ...
    public function testTheCharterOpensWithTheResponsibilityParagraph(): void
    {
        ...
        $this->assertStringContains('portable document-numbering package', $summary, 'The responsibility.');
        $this->assertStringContains('ships no allocator implementation', $summary, 'The non-goal.');
    }

    /**
     * The public API document states the grammar and bounds a host validates against, in one table.
     *
     * @return  void
     *
     * @since   0.1.0
     */
    public function testThePublicApiDocumentStatesTheGrammarAndBounds(): void
    {
        $document = $this->read('docs/public-api.md');

        foreach (['`MAXIMUM_PREFIX`', '`MAXIMUM_PADDING`', '`MAXIMUM_LENGTH`', 'A-Z', 'YYYY-MM'] as $needle) {
            $this->assertStringContains($needle, $document, 'The grammar table names ' . $needle . '.');
        }
        $this->assertStringContains('gapless-on-commit', $document, 'The allocation promise is named.');
    }
}
