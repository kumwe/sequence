...
$workspace = rtrim(sys_get_temp_dir(), '/\\') . '/kumwe-sequence-consumer-' . bin2hex(random_bytes(6));
...
$archive = $distDirectory . '/kumwe-sequence.zip';
if (!is_file($archive)) {
...
$consumerMetadata = [
    ...
    'license' => 'proprietary',
    'require' => ['kumwe/sequence' => $release],
    'repositories' => [
...
$installed = $consumer . '/vendor/kumwe/sequence';
...
consumerRun([PHP_BINARY, $installed . '/examples/allocate-and-render.php', $autoload], $workspace);
...
foreach (array_keys($classmap) as $symbol) {
    if (
        ...
        || str_starts_with($symbol, 'Kumwe\\Sequence\\Tests\\')
    ) {
