 ...
 * no framework. The layers keep one dependency direction — values stand alone, the contract may name the
 * refusal it raises, the refusal names nothing — and no source file selects an implementation at runtime.
 * Composer metadata keeps the runtime requirement at PHP alone and preserves the repository license.
 *
 * @since 0.1.0
 */
...
if (($composer['name'] ?? null) !== 'kumwe/sequence') {
    $errors[] = 'composer.json must name the package kumwe/sequence.';
}
...
if (($composer['autoload'] ?? null) !== ['psr-4' => [$namespaceRoot . '\\' => 'src/']]) {
    $errors[] = 'composer.json must autoload exactly the one canonical namespace Kumwe\\Sequence\\ from src/.';
}
...
echo 'Architecture verified: ' . count($files)
    . " source files under Kumwe\\Sequence, no host, driver or container coupling.\n";
